...
class Konten extends CI_Controller {
    ...
    public function update (){
        // var_dump($this->input->post('nama_foto'));
        $namafoto =$this->input->post('nama_foto');
        // var_dump($namafoto);
        // die;
        $fotoBaru = date('Ymdhis') . '.jpg';
        $config['upload_path']      = 'assets/upload/konten';
        $config['max_size'] = 500 * 1024; //3 * 1024 * 1024; //3Mb; 0=unlimited
        ...
        if($_FILES['foto']['size'] >= 500 * 1024){
            ...
            $filename=FCPATH.'/assets/upload/konten/'.$namafoto;
        ...
        $this->db->from('konten');
        $this->db->where('judul',$this->input->post('judul'));
        $this->db->where('foto !=',$namafoto);
        $cek = $this->db->get()->result_array(); 
        if($cek<>NULL){
            $this->session->set_flashdata('alert','
        <div class="alert alert-success alert-dismissible fade show" role="alert">
        <i class="fa fa-exclamation-circle me-2"></i>Judul sudah ada.
             <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>
        ');
        redirect('admin/konten');
            ...
            $this->session->set_flashdata('alert','
            <div class="alert alert-success alert-dismissible fade show" role="alert">
            <i class="fa fa-exclamation-circle me-2"></i>Berhasil mengubah konten.
                 <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>
            '); 
            redirect('admin/konten');  
